...
def preprocess_data(df):
   ...
   result_types = ['reliability_cn', 'gis_statistics', 'round_trip_reliability', 
                   ...
                   'delay_spread', 'composite', 'static_composite_cinr', 'catpsim_den_fail_areas']
   ...
   layer_types = ['terrain', 'site_area_reliability', 'cell_area_reliability', 
                  ...
                  'delay_spread', 'site_area_reliability_car']
   ...
   analysis_types = ['TdmaReliabilityDom', 'LteReliabilityDom', 'R6602ContoursDom', 
                     ...
                     'AnalogReliabilityDom', 'TrafficDom']
   ...
   # Task complexity score - optimized with more non-linear relationships
   df['TaskComplexityScore'] = (df['Resolution'] ** 1.2) * (df['SitesTotal'] ** 0.9) * \
                              (df['ResultsCount'] ** 0.8) * (np.maximum(df['SubscibersUsed'], 1) ** 0.7) * \
                              (np.maximum(df['DirectionCount'], 1) ** 0.6) * \
                              (np.log1p(df['EvalAreaInSquareKilometers']) ** 1.1) / \
                              (np.maximum(df['Cores'], 1) ** 0.9)
   ...
   df['SitesTotalCubed'] = df['SitesTotal'] ** 3
   df['ResolutionCubed'] = df['Resolution'] ** 3
   
   # Root terms for key features
   df['SitesTotalSqrt'] = np.sqrt(np.maximum(df['SitesTotal'], 0))
   df['ResolutionSqrt'] = np.sqrt(np.maximum(df['Resolution'], 0))
   df['AreaSqrt'] = np.sqrt(np.maximum(df['EvalAreaInSquareKilometers'], 0))
   ...
   # Feature for very large area tasks
   df['VeryLargeArea'] = (df['EvalAreaInSquareKilometers'] > 20000).astype(int)
   ...
   # Feature for very complex tasks
   df['VeryComplexTask'] = ((df['Resolution'] > 10) & (df['SitesTotal'] > 20) & 
                           (df['EvalAreaInSquareKilometers'] > 5000)).astype(int)
   ...
   # TaskState encoding
   if 'TaskState' in df.columns:
       df['TaskStateNumeric'] = pd.to_numeric(df['TaskState'], errors='coerce').fillna(1)
...
def get_feature_lists():
   ...
   base_features = [
       ...
       'LogAreaSquared', 'SitesTotalCubed', 'ResolutionCubed', 'SitesTotalSqrt',
       'ResolutionSqrt', 'AreaSqrt', 'SiteAreaInteraction', 'ResolutionResultsInteraction',
       'ComplexityScore2', 'MultiDirectional', 'HighResolution', 'LargeArea',
       'VeryLargeArea', 'ComplexTask', 'VeryComplexTask', 'WorkspaceHash', 
       'ModeNumeric', 'TaskStateNumeric'
   ]
...
def build_lgbm_model(params=None):
   if params is None:
       params = {
...
def build_gb_model(params=None):
   if params is None:
       params = {
           'n_estimators': 500,
           'learning_rate': 0.01,
...
def main():
   try:
       ...
       perform_cross_validation(X, y_log, y)  # Re-enabled for MAPE output
       ...
       test_preds = train_and_predict(X, y_log, X_test)
       ...
       np.savetxt('prediction1c.txt', test_preds, fmt='%.6f')
